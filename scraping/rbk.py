...
html_text = requests.get(conf.urlRBC, headers=conf.headers).text
...
def scraping_news(buff_number):
    ...
    for enum_data in data:
        id += 1
        news_repository[id] = {}
        news_headline = enum_data.find('span', class_='item__title rm-cm-item-text').text
        link_to_news = enum_data.find('a', class_='item__link').get('href')
